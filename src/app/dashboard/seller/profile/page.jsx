...
import { Loader2, Camera, User, Phone, MapPin, Lock, Save, Eye, EyeOff } from "lucide-react";
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-green-50">
              <span className="text-xs font-bold text-emerald-800 uppercase tracking-wider m-2 p-4">Profile photo</span>
            </div>
            <div className="p-5 flex items-center gap-5">
              <div className="relative w-20 h-20 flex-shrink-0">
                {form.photoPreview ? (
                  <img src={form.photoPreview} alt="preview" className="w-full h-full rounded-full object-cover border-2 border-emerald-200" />
                ) : (
                  <div className="w-full h-full rounded-full bg-slate-100 border-2 border-dashed border-gray-300 flex items-center justify-center">
          ...
          <button
            type="submit"
            disabled={loading}
            className="text-black w-full h-12 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed mb-3 pb-4 font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 hover:shadow-emerald-300 hover:scale-[1.02] transition-all"
          >
            {loading
              ? <><Loader2 size={16} className="animate-spin" /> Saving...</>
              : <><Save size={16} color="black"  /> Save changes</>
            }
          </button>
