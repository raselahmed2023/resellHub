...
function CheckoutForm({ product, clientSecret }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="p-4">
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 flex flex-col gap-1.5">
            <div className="flex justify-between text-xs text-gray-500">
              <span>Product price</span><span>৳{Number(product.price).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-xs text-gray-500">
              <span>Delivery charge</span><span>৳100</span>
            </div>
            <div className="flex justify-between text-sm font-bold text-gray-900 pt-1 border-t border-gray-100 mt-1">
              <span>Total</span><span>৳{(Number(product.price) + 100).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="p-5 flex flex-col gap-3">
          {[
            ...
            { key: "phone", label: "Phone number", icon: Phone, type: "tel", placeholder: "+880 1X XX XXX XXX" },
            { key: "address", label: "Delivery address", icon: MapPin, type: "text", placeholder: "Your full address" },
            ...
            <div key={key} className="flex flex-col gap-1.5">
              ...
              <input
                ...
                value={delivery[key]} onChange={(e) => set(key, e.target.value)}
      ...
      >
        {loading ? <><Loader2 size={16} className="animate-spin" /> Processing...</> : <>Pay ৳{(Number(product.price) + 100).toLocaleString()}</>}
      </button>
    </form>
  );
}
